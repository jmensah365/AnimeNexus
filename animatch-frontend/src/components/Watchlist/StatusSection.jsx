...
export default function StatusSection({ title, items, onClick, setDeleteTarget }) {
    ...
    return (
        <div className="mb-6">
            <button
                ...
                className="flex items-center justify-between w-full text-left px-2 py-3 bg-zinc-900 rounded-lg hover:bg-zinc-800 transition-colors"
            >
            ...
            {isOpen && (
                <div className="mt-4 grid grid-cols-1 sm:gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    {items.map((watchlist) => (
                        <AnimeCard key={watchlist.id} watchlist={watchlist} onClick={onClick} setDeleteTarget={setDeleteTarget} />
                    ))}
                </div>
            )}
        </div>
